...
# Open the dashboard for logged-in user
def open_dashboard(account):
    ...
    # Functions for deposit, withdrawal, and transfer
    ...
    def transfer_funds():
        ...
        if amount <= 0:
            ...
            return
        if not transfer_to_account.isdigit() or len(transfer_to_account) != 10:
            messagebox.showerror("Invalid Account", "Please enter a valid 10-digit account number.")
            return
        conn = sqlite3.connect('banking_app.db', timeout=10)
        cursor = conn.cursor()
        cursor.execute("SELECT balance FROM accounts WHERE account_number=?", (account_number,))
    ...
    # Radio buttons to select transaction type (Deposit, Withdraw, Transfer)
    ...
    select_transaction_type()  # Initial call to set up the default transaction view
